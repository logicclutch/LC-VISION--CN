...
import { UserCheck, ShieldAlert, Fingerprint, History } from 'lucide-react';
    ...
    <div className="max-w-[1600px] mx-auto pb-24 h-full flex flex-col space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-sm text-slate-400 font-medium mt-1">Facial re-identification and automated attendance.</p>
        </div>
      </motion.div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: "Authorized Staff", val: "42", icon: UserCheck, color: "text-emerald-400" },
          { title: "Unknown Persons", val: "3", icon: ShieldAlert, color: "text-orange-400" },
          { title: "Identifications", val: "892", icon: Fingerprint, color: "text-blue-400" },
          { title: "Total Logs", val: "1,204", icon: History, color: "text-slate-400" },
        ].map((stat, i) => (
          <motion.div 
            key={i}
            initial={{ opacity: 0, y: 20 }}
      ...
      >
         <div className="text-center text-slate-500 font-bold uppercase tracking-widest">
            Detailed attendance logs and profile matching data will appear here.
         </div>
      </motion.div>
    </div>
  );
