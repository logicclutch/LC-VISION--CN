...
import { Flame, ShieldAlert, HeartPulse, HardHat } from 'lucide-react';
    ...
    <div className="max-w-[1600px] mx-auto pb-24 h-full flex flex-col space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-sm text-slate-400 font-medium mt-1">Fire, smoke, and industrial hazard monitoring.</p>
        </div>
      </motion.div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: "Fire Detections", val: "0", icon: Flame, color: "text-red-400" },
          { title: "Smoke Detections", val: "0", icon: ShieldAlert, color: "text-slate-400" },
          { title: "PPE Violations", val: "2", icon: HardHat, color: "text-amber-400" },
          { title: "Fall Detections", val: "0", icon: HeartPulse, color: "text-purple-400" },
        ].map((stat, i) => (
          <motion.div 
            key={i}
            initial={{ opacity: 0, y: 20 }}
      ...
      >
         <div className="text-center text-slate-500 font-bold uppercase tracking-widest">
            Hazard history logs and video evidence will appear here.
         </div>
      </motion.div>
    </div>
  );
